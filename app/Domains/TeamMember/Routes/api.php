...
Route::middleware(['auth:sanctum', 'permission.map'])->prefix('team_member')->controller(TeamMemberController::class)->group(function () {
    Route::get('/', 'list_teamMember');
    Route::get('/{id}', 'get_teamMember');
});

Route::middleware(['auth:sanctum', 'permission.map'])->prefix('team_member')->controller(TeamMemberContentController::class)->group(function () {
